...
obj_color = {}
CHUNK_SIZE = 180 
OVERLAP_SIZE = 50 # Use 30 frames of overlap
...
def hex_to_bgr(hex_color: str) -> tuple:
    ...
    if len(hex_color) != 6: return 0, 255, 0
...
async def startup_event():
    if predictor is None: raise RuntimeError("MedSAM2 predictor could not be initialized.")
...
async def create_session_upload(video_file: UploadFile = File(...)):
    ...
    try:
        ...
        with open(video_path, "wb") as f: f.write(await video_file.read())
        cap = cv2.VideoCapture(video_path)
        frame_count = int(cap.get(cv2.CAP_PROP_FRAME_COUNT))
        session_states[session_id] = {
            "video_path": video_path, "session_dir": session_dir,
            "original_width": int(cap.get(cv2.CAP_PROP_FRAME_WIDTH)),
            "original_height": int(cap.get(cv2.CAP_PROP_FRAME_HEIGHT)),
            "frame_count": frame_count, "fps": cap.get(cv2.CAP_PROP_FPS), "user_points": {},
        }
        cap.release()
        return CreateSessionResponse(session_id=session_id, message="Session created.", frame_count=frame_count, fps=session_states[session_id]["fps"])
    except Exception as e:
...
async def get_frame(session_id: str, frame_index: int = Path(..., ge=0)):
    ...
    cap.release()
    if not success: raise HTTPException(status_code=404, detail="Could not read frame.")
...
async def add_new_points(data: ClickData):
    ...
    if not session: raise HTTPException(status_code=404, detail="Session ID not found.")
    frame_index_str = str(data.frameIndex)
    if frame_index_str not in session["user_points"]: session["user_points"][frame_index_str] = []
    session["user_points"][frame_index_str].append({"objectId": data.objectId, "points": data.points, "labels": data.labels})
    if data.color: obj_color[data.objectId] = hex_to_bgr(data.color)
    cap = cv2.VideoCapture(session["video_path"])
    cap.set(cv2.CAP_PROP_POS_FRAMES, data.frameIndex)
    success, input_image = cap.read()
    cap.release()
    if not success: raise HTTPException(status_code=404, detail="Frame not found for preview.")
    ...
    for p_data in session["user_points"][frame_index_str]:
        ...
        if obj_id not in current_frame_annotations: current_frame_annotations[obj_id] = {'points': [], 'labels': []}
        ...
        with torch.inference_mode():
            for obj_id, annotation_data in current_frame_annotations.items():
                _, all_out_obj_ids, all_out_mask_logits = predictor.add_new_points(
                    inference_state=preview_state, frame_idx=0, obj_id=obj_id,
...
async def process_video(session_id: str):
    session = session_states.get(session_id)
    if not session or not session.get("user_points"): raise HTTPException(status_code=404, detail="Session or user points not found.")
    
    video_path, session_dir = session["video_path"], session["session_dir"]
    frame_count, H, W = session["frame_count"], session["original_height"], session["original_width"]
    ...
    last_chunk_hand_off_info = None # This will store bounding boxes {obj_id: [x1, y1, x2, y2]}
    ...
    for effective_start_frame in range(0, frame_count, EFFECTIVE_CHUNK_SIZE):
        ...
        if actual_start_frame >= chunk_end_frame: continue

        logger.info(f"--- Processing frames {actual_start_frame} to {chunk_end_frame-1} ---")
        ...
        for i in range(actual_start_frame, chunk_end_frame):
            ...
            cv2.imwrite(os.path.join(chunk_frames_dir, f"{i - actual_start_frame:05d}.jpg"), frame)
        cap_extract.release()
        ...
        with torch.autocast(device_type="cuda", dtype=torch.bfloat16):
            ...
            if effective_start_frame == 0:
                ...
                for frame_idx_str, points_data_list in session["user_points"].items():
                    relative_frame_idx = int(frame_idx_str)
                    if 0 <= relative_frame_idx < (chunk_end_frame - actual_start_frame):
                        for p_data in points_data_list:
                            predictor.add_new_points(
                                inference_state=inference_state, frame_idx=relative_frame_idx, obj_id=p_data['objectId'],
                                points=np.array(p_data['points'], dtype=np.float32), labels=np.array(p_data['labels'], dtype=np.int32)
                            )
            elif last_chunk_hand_off_info is not None:
                logger.info(f"Initializing chunk with bounding box from previous chunk.")
                prompt_frame_idx = OVERLAP_SIZE - 1
                for obj_id, bbox in last_chunk_hand_off_info.items():
                    points = [[bbox[0], bbox[1]], [bbox[2], bbox[3]]]
                    labels = [2, 3] # Label 2 for top-left, 3 for bottom-right
                    predictor.add_new_points(
                        inference_state=inference_state, frame_idx=prompt_frame_idx,
                        obj_id=obj_id, points=np.array(points, dtype=np.float32),
                        labels=np.array(labels, dtype=np.int32)
                    )

            # Propagate through the chunk
            with torch.inference_mode():
                for relative_frame_idx, out_obj_ids, out_mask_logits in predictor.propagate_in_video(inference_state):
                    absolute_frame_idx = actual_start_frame + relative_frame_idx
                    if absolute_frame_idx >= chunk_end_frame: break
                    if absolute_frame_idx < effective_start_frame: continue
                    
                    total_frames_processed += 1
                    frame_path = os.path.join(chunk_frames_dir, f"{relative_frame_idx:05d}.jpg")
                    ...
                    for idx, obj_id in enumerate(out_obj_ids):
                        reshaped_mask = np.reshape((out_mask_logits[idx] > 0.0).cpu().numpy(), (H, W))
                        ...
                        if absolute_frame_idx == chunk_end_frame - 1:
                            ...
                            if len(coords) > 0:
                                ...
                                last_chunk_hand_off_info[obj_id] = [x_min, y_min, x_max, y_max]
    ...
    if not os.path.exists(output_video_path) or total_frames_processed == 0:
        raise HTTPException(status_code=500, detail="Output video could not be generated. Model failed to track object.")
...
async def delete_session(session_id: str):
    ...
    if session_dir and os.path.exists(session_dir): shutil.rmtree(session_dir)
