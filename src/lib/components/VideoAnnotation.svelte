<script lang="ts">
	...
	let annotationSync: AnnotationSync;

	// Generate unique session ID
	const sessionId = `session_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
	const videoId = `video_${uploadedVideo?.name?.replace(/[^a-zA-Z0-9]/g, '_')}_${Date.now()}`;

	onMount(() => {
		// Initialize annotation sync
		annotationSync = new AnnotationSync();
		annotationSync.connect(sessionId);
		isConnected = true;

		// Listen for annotations from other users (in a collaborative environment)
		annotationSync.onAnnotation((annotation) => {
			console.log('Received annotation from another user:', annotation);
			// In a real app, you might want to display these differently
		});

		return () => {
			annotationSync?.disconnect();
		};
	});
	...
	async function handleAnnotationCreated(event: CustomEvent<any>) {
		const annotation = event.detail;
		annotations = [...annotations, annotation];

		// Send annotation to backend with additional metadata
		...
		try {
			const result = await sendAnnotation(annotationData);
			if (result.success) {
				console.log('Annotation synced with backend:', result.id);
			} else {
				console.warn('Failed to sync annotation with backend');
...
<div class="mx-auto max-w-7xl space-y-8">
	...
	<div class="flex items-center justify-between">
		<button
			on:click={handleBack}
			class="text-medical-600 hover:text-medical-800 flex items-center transition-all duration-200 hover:translate-x-1"
		>
		...
		<div class="flex items-center space-x-4">
			...
			<button on:click={processSAM2} class="btn-primary flex items-center" disabled={isProcessing}>
				<Sparkles class="mr-2 h-4 w-4" />
				{isProcessing ? 'Processing...' : 'Process with SAM2'}
			</button>
			<button on:click={downloadMasks} class="btn-outline flex items-center">
				<Download class="mr-2 h-4 w-4" />
				Download Prompts
			</button>
			<button on:click={clearAnnotations} class="btn-secondary flex items-center">
					...
					<div class="flex items-center justify-between">
						...
						<div class="flex items-center space-x-2">
							<button
								...
								class="bg-medical-100 hover:bg-medical-200 text-medical-700 flex items-center rounded-lg px-3 py-2 text-sm transition-colors"
							>
								{#if showPreview}
				...
				<div class="control-panel space-y-6">
					...
					<div class="flex items-center justify-center space-x-6">
						<button
							...
							class="text-medical-600 hover:text-medical-800 hover:bg-medical-100 rounded-full p-3 transition-all duration-200 hover:scale-110"
						>
						...
						<button
							on:click={togglePlay}
							class="bg-primary-500 hover:bg-primary-600 rounded-full p-4 text-white shadow-lg transition-all duration-200 hover:scale-110"
						>
							{#if isPlaying}
								<Pause class="h-6 w-6" />
							{:else}
					...
					<div class="border-medical-200 flex items-center justify-center space-x-4 border-t pt-6">
						<button
							...
							class="flex items-center px-6 py-3 {annotationMode
						...
						{#if annotationMode}
							<div class="animate-fade-in flex items-center space-x-2">
								<button
									...
									class="flex items-center px-4 py-3 {currentAnnotationType === 'positive'
								...
								<button
									...
									class="flex items-center px-4 py-3 {currentAnnotationType === 'negative'
								...
								<button
									...
									class="flex items-center px-4 py-3 {currentAnnotationType === 'box'
