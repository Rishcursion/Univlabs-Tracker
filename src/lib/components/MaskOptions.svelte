...
<div class="card">
	...
	<div class="space-y-4">
		<!-- Custom Annotation -->
		<div
			class="cursor-pointer rounded-lg border-2 p-4 transition-all duration-200 {selectedOption ===
			'custom'
		...
		>
			<div class="mb-2 flex items-center">
				<div
					class="bg-primary-100 text-primary-600 mr-3 flex h-8 w-8 items-center justify-center rounded-lg"
				>
					<Wand2 class="h-4 w-4" />
				</div>
		...
		{#if currentPresets.length > 0}
			...
			>
				...
				<p class="text-medical-600 mb-3 text-sm">
					Pre-configured prompts for common anatomical structures
				</p>
		...
		>
			...
			<p class="text-medical-600 text-sm">
				Let SAM2 automatically detect and segment all objects in the frame
			</p>
		</div>
	</div>
	...
	{#if selectedOption === 'custom'}
		<div class="mt-4 rounded-lg bg-blue-50 p-3">
			<p class="text-sm text-blue-800">
				<strong>SAM2 Interactive Mode:</strong> Use positive points (green +) to indicate areas to include,
				negative points (red -) to exclude areas, and bounding boxes to roughly outline objects.
			</p>
		</div>
	{:else if selectedOption === 'preset'}
		<div class="mt-4 rounded-lg bg-teal-50 p-3">
			<p class="text-sm text-teal-800">
				<strong>Anatomical Templates:</strong> Pre-configured prompts optimized for common surgical structures
	...
	{:else if selectedOption === 'auto'}
		<div class="mt-4 rounded-lg bg-green-50 p-3">
			<p class="text-sm text-green-800">
				<strong>Everything Mode:</strong> SAM2 will automatically generate masks for all detectable objects
